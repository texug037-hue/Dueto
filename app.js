// ============================================================
// DUETO — versão com Firebase de verdade (Auth por SMS + Firestore)
// ============================================================
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('loginPhone').addEventListener('keydown', (e) => { if (e.key === 'Enter') enviarCodigo(); });
  document.getElementById('btnConfirmCode').addEventListener('click', confirmarCodigo);
  document.getElementById('loginCode').addEventListener('keydown', (e) => { if (e.key === 'Enter') confirmarCodigo(); });
...
onAuthStateChanged(auth, (user) => {
  if (user) {
    const info = NUMEROS_AUTORIZADOS[user.phoneNumber];
    if (!info) { signOut(auth); return; }
    currentUser = { uid: user.uid, phone: user.phoneNumber, nome: info.nome, codigo: info.codigo };
    ...
    document.getElementById('stepCode').classList.add('hide');
    document.getElementById('stepPhone').classList.remove('hide');
  }
});
...
async function confirmPay(btn){
  ...
  try {
    if (parcelas && parcelas.atual < parcelas.total) {
      ...
      if (parcelas) {
        await addDoc(collection(db, 'historico'), {
          nome: `${c.nome} — parcela ${parcelas.atual}/${parcelas.total} (última)`,
          valor: val, data: new Date().toISOString().slice(0,10), categoria: c.categoria,
          pagoPor: currentUser.phone, criadoEm: serverTimestamp()
        });
      }
