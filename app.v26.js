...
const authPronto = setPersistence(auth, inMemoryPersistence)
  .then(() => signOut(auth))
  .then(() => { authInicializado = true; })
  .catch((e) => { console.error('Inicialização do login:', e); authInicializado = true; });
...
let contasCache = [];
let historicoCache = [];
let unsubContas = null, unsubHistorico = null, unsubNotificacoes = null;
let editingId = null;
let uploadTarget = null;  // { tipo:'card'|'parcela', id }
let filtroStatusAtivo = null; // 'vencidas' | 'hoje' | 'mes' | null
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('btnAtivarBiometria').addEventListener('click', () => {
    if (currentUser && senhaSessaoAtual) ativarBiometriaNesteAparelho(currentUser.phone, senhaSessaoAtual);
    else showToast('Faz login com a senha uma vez antes de ativar a biometria.');
  });
  document.getElementById('btnNotifBell').addEventListener('click', abrirNotificacoes);
  document.getElementById('btnCloseNotif').addEventListener('click', fecharNotificacoes);
  document.getElementById('btnClearNotif').addEventListener('click', limparNotificacoesDesteUsuario);
  document.getElementById('notifOverlay').addEventListener('click', fecharNotificacoes);
  ...
  document.getElementById('ovalVencidas').addEventListener('click', () => mostrarStatus('vencidas'));
  document.getElementById('ovalHoje').addEventListener('click', () => mostrarStatus('hoje'));
  document.getElementById('ovalMes').addEventListener('click', () => mostrarStatus('mes'));
...
let senhaSessaoAtual = null;
let loginSolicitado = false;
...
function contextoSeguroParaBiometria(){
...
function atualizarBotaoBiometria(){
  ...
  if (suportaBiometria() && contextoSeguroParaBiometria()) {
    btn.classList.remove('hide');
    btn.textContent = saved && NUMEROS_AUTORIZADOS[saved.phone]
      ? `👆 Entrar como ${NUMEROS_AUTORIZADOS[saved.phone].nome}`
      : '👆 Entrar com digital';
  } else {
...
async function cadastrarBiometriaPelaTelaInicial(){
  const e164 = '+55' + normalizePhone(document.getElementById('loginPhone').value);
  const senha = document.getElementById('loginPassword').value;
  const err = document.getElementById('loginError');
  err.textContent = '';
  if (!NUMEROS_AUTORIZADOS[e164]) { err.textContent = 'Informe um dos números autorizados.'; return; }
  if (!senha) { err.textContent = 'Digite a senha antes de adicionar a digital.'; return; }
  err.textContent = 'Confirme a digital no aparelho...';
  await ativarBiometriaNesteAparelho(e164, senha);
  err.textContent = '';
}

function atualizarBotaoAtivarBiometria(){
  const btn = document.getElementById('btnAtivarBiometria');
  if (!btn || !currentUser) return;
  const saved = credenciaisSalvas();
  const jaAtivada = saved && saved.phone === currentUser.phone;
  btn.classList.toggle('hide', !suportaBiometria() || jaAtivada);
}

async function ativarBiometriaNesteAparelho(e164, senha){
  if (!suportaBiometria()) { showToast('Esse navegador não suporta biometria.'); return; }
  if (!contextoSeguroParaBiometria()) { showToast('A digital só funciona pelo endereço HTTPS do aplicativo.'); return; }
  try {
    const disponivel = await PublicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable();
    if (!disponivel) { showToast('Esse aparelho não tem biometria configurada.'); return; }
...
async function entrarComBiometria(){
  const saved = credenciaisSalvas();
  const err = document.getElementById('loginError');
  if (!saved || !saved.credId || !saved.phone || !saved.senha) {
    err.textContent = 'Este aparelho ainda não tem um login biométrico vinculado.';
    return;
  }
  if (!contextoSeguroParaBiometria()) { err.textContent = 'A digital só funciona pelo endereço HTTPS do aplicativo.'; return; }
  try {
    err.textContent = 'Toque no sensor de digital do celular...';
    loginSolicitado = true;
    const publicKey = {
      challenge: crypto.getRandomValues(new Uint8Array(32)),
    ...
    // Quando o cadastro local existe, informar o ID faz o Android abrir
    // diretamente a biometria, sem mostrar a tela "Usar chave de acesso salva?".
    if (saved && saved.credId) {
      const credIdBytes = Uint8Array.from(atob(saved.credId), c => c.charCodeAt(0));
      publicKey.allowCredentials = [{ id: credIdBytes, type: 'public-key' }];
    }
    try {
      await navigator.credentials.get({ publicKey });
    } catch (firstError) {
      // Algumas versões do Chrome/Android não aceitam o ID salvo depois de
      // uma atualização do domínio. Tenta novamente pelo autenticador do
      // sistema, sem limitar a uma credencial antiga.
      if (firstError.name !== 'NotAllowedError') throw firstError;
      delete publicKey.allowCredentials;
      await navigator.credentials.get({ publicKey });
    }
    // biometria confirmada pelo Android -> completa o login da conta deste aparelho
    await entrarComSenha(saved.phone, saved.senha, err);
  } catch (e) {
    console.error(e);
    loginSolicitado = false;
    if (e.name === 'NotAllowedError') {
      err.textContent = 'A leitura foi cancelada ou não foi reconhecida. Tente novamente.';
    } else {
      err.textContent = 'Não foi possível abrir a digital neste aparelho. Cadastre novamente ou use a senha.';
    }
  }
}
...
onAuthStateChanged(auth, (user) => {
  ...
  if (user) {
    if (!loginSolicitado) { signOut(auth); return; }
    loginSolicitado = false;
    ...
    startListening();
    senhaSessaoAtual = senhaDigitadaAgora || senhaSessaoAtual;
    senhaDigitadaAgora = null;
    atualizarBotaoAtivarBiometria();
  } else {
    currentUser = null;
    senhaSessaoAtual = null;
...
function startListening(){
  ...
  // Não ordenar no servidor: serverTimestamp pode ficar null por alguns
  // instantes e impedir que o outro usuário receba a notificação.
  const qNotif = query(collection(db, 'notificacoes'));
  unsubNotificacoes = onSnapshot(qNotif, (snap) => {
    const uid = currentUser && currentUser.uid;
    const lista = snap.docs.map(d => ({ id: d.id, ...d.data() }))
      .filter(n => !uid || !(n.ocultasPor || []).includes(uid))
      .sort((a, b) => dataNotificacao(b.quando) - dataNotificacao(a.quando));
    window.notificacoesCache = lista;
    notifNaoLidas = lista.length;
    atualizarBadgeNotif();
    if (document.getElementById('notifSheet').classList.contains('show')) renderNotifList();
  }, (err) => {
    console.error('Notificações:', err);
    showToast('Não foi possível sincronizar as notificações. Publique as regras do Firestore atualizadas.');
  });
}

function dataNotificacao(valor){
  if (!valor) return 0;
...
async function notificar(titulo, corpo){
  ...
  try {
    await addDoc(collection(db, 'notificacoes'), {
      titulo, corpo, quando: new Date(), criadoPor: currentUser.uid, ocultasPor: []
    });
  } catch (e) { console.error('Não consegui compartilhar a notificação:', e); }
  registrarNotificacaoNoFeed(titulo, corpo);
  if (!('Notification' in window) || Notification.permission !== 'granted') {
    showToast(`${titulo}: ${corpo}`);
    return;
  }
  const opcoes = { body: corpo, icon: './icon-192.png', badge: './badge-96-v2.png', vibrate: [200, 100, 200] };
  try {
...
// ---------- sino de notificações dentro do app ----------
const NOTIF_FEED_KEY = 'dueto_notif_feed';
let notifNaoLidas = 0;
window.notificacoesCache = [];

function carregarFeedNotif(){
  try { return JSON.parse(localStorage.getItem(NOTIF_FEED_KEY)) || []; }
  catch (e) { return []; }
}
function registrarNotificacaoNoFeed(titulo, corpo){
  const lista = carregarFeedNotif();
  lista.unshift({ titulo, corpo, quando: new Date().toISOString() });
  localStorage.setItem(NOTIF_FEED_KEY, JSON.stringify(lista.slice(0, 30)));
  notifNaoLidas++;
  atualizarBadgeNotif();
}
function atualizarBadgeNotif(){
  ...
  if (!badge) return;
  if (notifNaoLidas > 0) { badge.textContent = notifNaoLidas > 9 ? '9+' : notifNaoLidas; badge.classList.remove('hide'); }
...
function renderNotifList(){
  const lista = window.notificacoesCache || [];
  const body = document.getElementById('notifList');
  body.innerHTML = lista.length ? lista.map(n => `
    <div class="resumo-item" style="flex-direction:column; align-items:flex-start; gap:3px;">
      <strong style="font-size:12.5px; color:var(--text);">${n.titulo}</strong>
      <span style="font-size:11.5px; color:var(--muted);">${n.corpo}</span>
      <span style="font-size:10px; color:var(--muted); font-family:var(--f-mono);">${n.quando?.toDate ? n.quando.toDate().toLocaleString('pt-BR') : 'agora'}</span>
    </div>`).join('') : '<div class="empty-state">Nenhuma notificação ainda.</div>';
}
async function limparNotificacoesDesteUsuario(){
  if (!currentUser || !window.notificacoesCache.length) return;
  try {
    await Promise.all(window.notificacoesCache.map(n =>
      updateDoc(doc(db, 'notificacoes', n.id), { ocultasPor: arrayUnion(currentUser.uid) })
    ));
    showToast('Notificações limpas só neste login.');
  } catch (e) { console.error(e); showToast('Não consegui limpar as notificações.'); }
}
...
function renderList(){
  ...
  if (filtroStatusAtivo) { renderStatusView(list, filtroStatusAtivo); return; }
...
// ---------- telas puxadas pelos balões do topo (Vencidas / Vence hoje / Mês) ----------
...
function itensPorStatus(tipo){
  const hoje = REF_TODAY();
  if (tipo === 'vencidas') return contasCache.filter(c => !c.pago && diasAte(c.vencimento) < 0);
  if (tipo === 'hoje') return contasCache.filter(c => !c.pago && diasAte(c.vencimento) === 0);
  if (tipo === 'mes') {
    const anoMes = hoje.toISOString().slice(0, 7);
    return contasCache.filter(c => !c.pago && c.vencimento && c.vencimento.startsWith(anoMes));
  }
  return [];
}

function renderStatusView(list, tipo){
  const titulos = { vencidas: 'Contas vencidas', hoje: 'Vence hoje', mes: 'Contas do mês' };
  document.getElementById('contentTitle').textContent = titulos[tipo] || '';
  const itens = agruparVencidasPorCategoria(itensPorStatus(tipo));
  if (itens.length === 0) {
    list.innerHTML = '<div class="empty-state">Nada aqui.</div>';
    return;
  }
  list.innerHTML = itens.map(cardHtml).join('');
  attachCardListeners(list);
}
...
async function confirmSetupEntrada(id){
  ...
  try {
    await addDoc(collection(db, 'historico'), {
      ...
      comprovanteNome: c.comprovanteNome || null, comprovanteData: c.comprovanteData || null
    });
      ...
      await notificar('Pagamento registrado', `${c.nome} — entrada de R$ ${valorEntrada.toFixed(2).replace('.', ',')} por ${currentUser.nome}`);
  } catch (e) {
    console.error(e);
...
async function confirmPay(btn){
  ...
  try {
    if (parcelas && parcelas.atual < parcelas.total) {
      ...
      await notificar('Pagamento registrado', `${nomeLog} — R$ ${val.toFixed(2).replace('.', ',')} por ${currentUser.nome}`);
    } else {
      ...
      if (c.categoria === 'cartao' || c.categoria === 'geral') { reset.minimo = null; reset.parcelamento = null; reset.valorParcela = null; }
      await updateDoc(doc(db, 'contas', id), reset);
      await notificar('Pagamento registrado', `${nomeHist} — R$ ${val.toFixed(2).replace('.', ',')} por ${currentUser.nome}`);
    }
  } catch (e) {
    console.error(e);
...
function pickCat(btn){
  document.querySelectorAll('#segCat button').forEach(b => b.classList.remove('active'));
  btn.classList.add('active');
  toggleCartaoFields(btn.dataset.cat === 'cartao' || btn.dataset.cat === 'geral');
...
function toggleNomeField(cat){
  ...
  document.getElementById('minField').classList.toggle('hide', cat !== 'cartao');
}
...
async function saveEntry(){
  ...
  if ((categoria === 'cartao' || categoria === 'geral') && numParcelas >= 1) {
  ...
  const payload = { categoria, nome, valor, minimo: categoria === 'cartao' ? minimo : null, parcelamento, valorParcela, vencimento };
  ...
  try {
    if (editingId) {
      ...
      await notificar('Nova conta lançada', `${nome} — R$ ${valor.toFixed(2).replace('.', ',')} por ${currentUser.nome}`);
    }
    closeModal();
  } catch (e) {
    console.error(e);
...
function renderResumo(list){
  ...
  const acordeaoHtml = Object.keys(CAT_LABEL).map(cat => {
    ...
    const corpo = itens.length ? itens.map(c => {
      ...
      return `<div class="resumo-item" data-id="${c.id}">
  ...
  list.querySelectorAll('.resumo-item[data-id]').forEach(item => {
    item.style.cursor = 'pointer';
    item.addEventListener('click', () => abrirDetalheConta(item.dataset.id));
  });
}

// abre o card de verdade (mesmo visual/funcional da lista normal) dentro
// de uma folha, em vez de mandar direto pra edição
function abrirDetalheConta(id){
  const c = contasCache.find(x => x.id === id);
  if (!c) return;
  document.getElementById('histTitle').textContent = c.nome;
...
function setupBackButton(){
  ...
  window.addEventListener('popstate', () => {
    ...
    if (document.getElementById('notifSheet').classList.contains('show')) {
      fecharNotificacoes();
      history.pushState({ dueto: true }, '');
      return;
    }
